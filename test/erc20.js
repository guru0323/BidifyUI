...
contract("Bidify", (accounts) => {
  it("should support listings with an erc20", async () => {
    ...
    let listing = await Bidify.list(erc20.address, ERC721.address, token, web3.utils.toWei("1"),
  ...
  it("should allow bidding with an erc20", async () => {
    ...
    await erc20.approve(Bidify.address, await Bidify.getNextBid.call(0), { from: accounts[2] });
    let bid = await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[2] });

    // Ensure the contract has the correct balance
    assert.strictEqual((await erc20.balanceOf.call(Bidify.address)).toString(), web3.utils.toWei("1"));
    ...
    await erc20.approve(Bidify.address, await Bidify.getNextBid.call(0), { from: accounts[3] });
    await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[3] });
  ...
  it("should allow finishing auctions", async () => {
    ...
    await Bidify.finish(0, { from: accounts[4] });

    // Ensure the ERC20 was transferred
    assert.strictEqual((await erc20.balanceOf.call(Bidify.address)).toString(), "0");
  ...
  it("should calculate the correct price unit for various ERC20s", async () => {
    ...
    assert.strictEqual((await Bidify.getPriceUnit.call((await ERC20.new(0)).address)).toString(), "1");
    assert.strictEqual((await Bidify.getPriceUnit.call((await ERC20.new(2)).address)).toString(), "1");
    assert.strictEqual((await Bidify.getPriceUnit.call((await ERC20.new(4)).address)).toString(), "1");

    // The following should render with 0.0001
    assert.strictEqual((await Bidify.getPriceUnit.call((await ERC20.new(8)).address)).toString(), "10000");
    assert.strictEqual((await Bidify.getPriceUnit.call((await ERC20.new(18)).address)).toString(), "100000000000000");
  });
});
