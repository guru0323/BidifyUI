...
let Bidify = artifacts.require("Bidify");
let ERC721 = artifacts.require("ERC721");

let listStartTime;

contract("Bidify", (accounts) => {
  it("should set self to the owner", async () => {
    Bidify = await Bidify.new();
    assert.strictEqual(await Bidify.owner.call(), accounts[0]);
  });
  ...
  it("should support listings", async () => {
    ...
    await ERC721.approve(Bidify.address, token, { from: accounts[1] });

    let listing = await Bidify.list("0x0000000000000000000000000000000000000000",
    ...
    assert.strictEqual(await ERC721.ownerOf.call(token), Bidify.address);
    ...
    await ERC721.approve(Bidify.address, token, { from: accounts[2] });
    assert.strictEqual((await Bidify.list("0x0000000000000000000000000000000000000000",
                                              ERC721.address, token, web3.utils.toWei("1"),
                                              5, "0x0000000000000000000000000000000000000000",
  ...
  it("should allow bidding", async () => {
    let bid = await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[2], value: await Bidify.getNextBid.call(0) });
    // Ensure the contract has the ETH
    assert.strictEqual((await web3.eth.getBalance(Bidify.address)).toString(), web3.utils.toWei("1"));
    ...
    let listing = await Bidify.getListing.call(0);
    ...
    await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[3], value: (await Bidify.getNextBid.call(0)) });
    assert.strictEqual((await web3.eth.getBalance(Bidify.address)).toString(), web3.utils.toWei("2.05"));

    // Make sure the first bidder was paid back
    assert.strictEqual((await Bidify.balanceOf(accounts[2])).toString(), web3.utils.toWei("1"));
    await Bidify.withdraw(accounts[2], { from: accounts[9] });
    ...
    await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[2], value: (await Bidify.getNextBid.call(0)) });
    assert.strictEqual((await web3.eth.getBalance(Bidify.address)).toString(), web3.utils.toWei("2.1525"));

    // Verifies rounding was successfully performed
    await Bidify.bid(0, "0x0000000000000000000000000000000000000000", { from: accounts[3], value: (await Bidify.getNextBid.call(0)) });
    assert.strictEqual((await web3.eth.getBalance(Bidify.address)).toString(), web3.utils.toWei("3.3101"));

    // Verify the listing's final state is still accurate
    listing = await Bidify.getListing.call(0);
    assert.strictEqual(listing.creator, accounts[1]);
    assert.strictEqual(listing.currency, "0x0000000000000000000000000000000000000000");
    assert.strictEqual(listing.platform, ERC721.address);
    assert.strictEqual(listing.token.toString(), "100");
  ...
  it("should allow finishing auctions", async () => {
    ...
    let finish = await Bidify.finish(0, { from: accounts[4] });
    ...
    assert.strictEqual((await Bidify.balanceOf(accounts[0])).toString(), web3.utils.toWei("0.046304"));
    await Bidify.withdraw(accounts[0], { from: accounts[9] });
    assert.strictEqual((await Bidify.balanceOf(accounts[1])).toString(), web3.utils.toWei("1.111296"));
    await Bidify.withdraw(accounts[1], { from: accounts[9] });

    // Make sure the listing was updated
    assert.strictEqual((await Bidify.getListing.call(0)).paidOut, true);
